...
//        delay(1000L)
    ...
    //어디에도 속하지 않지만 원래부터 존재하는 전역 GlobalScope가 있다.
    //이 전역 스코프를 이용하면 코루틴을 쉽게 수행할 수 있다.
    //GlobalScope는 어떤 계층에도 속하지 않고 영원히 동작하게 된다는 문제점이 있다.
    //프로그래밍에서 전역 객체를 잘 사용하지 않는 것 처럼 GlobalScope도 잘 사용하지 않는다.
    fun `GlobalScope`() = run {
    ...
    suspend fun printRandom() {
        delay(500L)
        printLog(Random.nextInt(0, 500).toString())
    }


    //scope로 계층적으로 형성된 코루틴을 관리할 수 있다.
    //필요에 따라 코루틴 스코프를 관리할 수 있다.
    fun `CoroutineScope`() {
    ...
    fun `CEH_코루틴_익셉션_핸들러`() = runBlocking<Unit> {
    ...
    val ceh = CoroutineExceptionHandler { _, exception ->
        printLog("Something happend: $exception")
    }


    //runBlocking은 자식이 예외로 종료되면 항상 종료되고 CEH를 호출하지 않기에 CEH 를 사용할 수 없다.
    // 에러 발생할 것임..
    // java.lang.RuntimeException: Unable to start activity ComponentInfo{com.example.red_chapter1/com.example.red_chapter1.MainActivity}: java.lang.ArithmeticException
    // Caused by: java.lang.ArithmeticException
    fun `runBlocking과_CEH`() = runBlocking<Unit> {
        val job = launch(ceh) {
            val a = async<String> { printRandom1().toString() }
            val b = async<String> { printRandom2().toString() }
            printLog(a.await())
            printLog(b.await())
        }
        job.join()
    ...
    fun `SupervisorJob`() = runBlocking<Unit> {
        val scope = CoroutineScope(Dispatchers.IO + kotlinx.coroutines.SupervisorJob() + ceh)
    ...
    //코루틴 스코프와 슈퍼바이저 잡을 합친듯 한 SupervisorScope
    fun `SupervisorScope`() = runBlocking<Unit> {
        val scope = CoroutineScope(Dispatchers.IO)
    ...
    //슈퍼바이저 스코프를 사용할 때 주의점은 무조건 자식 수준에서 예외를 핸들링 해야한다.
